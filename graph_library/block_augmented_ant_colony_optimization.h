...
void ant_walk(
	...
	// Blockwise Optimization to update sequence and state_path
	std::vector<Edge> temp_sequence;
	for (int i = 0; i < m; i++) {
		temp_sequence.push_back(sequence[i]);
	}

	auto fcost = calculate_cost(graph, temp_sequence);
	cout << "First cost is " << fcost << endl;


	const int block_size = sqrt(m);
    auto blocks = divide_into_blocks(temp_sequence, block_size);
    for (size_t i = 0; i < blocks.size(); ++i) {
        blocks[i] = Optimize_Block(graph, blocks, i);
    }
    temp_sequence = concatenate_blocks(blocks);
    for(int i = 0; i < m ; i++) {
	...
	for (int i = 0; i < m; ++i) {
		if (direction[i] == 1) {
			state_path[i] = state_index(seq[i].second, seq[i].first, graph -> deliver_edges);
		}
		else {
			state_path[i] = state_index(seq[i].first, seq[i].second, graph -> deliver_edges);
		}
	}
}

// Ant Colony Optimization (ACO) with multi-threading
pair< vector<Edge>, double> Block_Augmented_Ant_Colony_Optimization_MultiThreads(
	Graph *graph, 
	const int k_max = 10, 
		...
		while (start < num_ants) {
			...
			for (int t = 0; t < count; ++t) {
				...
				if (cost[t] < result.second) {
					cout << "There is a better solution" << endl;
					result.first.clear();
					for (int i = 0; i < m; ++i) {
...
// Ant Colony Optimization (ACO)
pair< vector<Edge>, double> Block_Augmented_Ant_Colony_Optimization(
	Graph *graph, 
	const int k_max = 10, 
	const int num_ants = 10, 
	const bool verbose = false,
	...
	for (int k = 1; k <= k_max; ++k) {
		...
		for (int ant = 0; ant < num_ants; ++ant) {
			...
			cout << "Cost = " << cost << ", best = " << result.second << endl;
